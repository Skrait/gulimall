<template>
  <el-dialog
    ...
    <el-form-item label="层级(即几级分类)" prop="catLevel">
      <el-input v-model="dataForm.catLevel" placeholder="层级(即几级分类)"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          catLevel: [
            { required: true, message: '层级(即几级分类)不能为空', trigger: 'blur' }
          ],
          showStatus: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.catId) {
            this.$http({
              url: this.$http.adornUrl(`/product/pmscategorycopy1/info/${this.dataForm.catId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.pmsCategoryCopy1.name
                this.dataForm.parentCid = data.pmsCategoryCopy1.parentCid
                this.dataForm.catLevel = data.pmsCategoryCopy1.catLevel
                this.dataForm.showStatus = data.pmsCategoryCopy1.showStatus
                this.dataForm.sort = data.pmsCategoryCopy1.sort
                this.dataForm.icon = data.pmsCategoryCopy1.icon
                this.dataForm.productUnit = data.pmsCategoryCopy1.productUnit
                this.dataForm.productCount = data.pmsCategoryCopy1.productCount
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/product/pmscategorycopy1/${!this.dataForm.catId ? 'save' : 'update'}`),
              method: 'post',
